...
class TranslationQueryTest {
  ...
  void translationMap() {
    GraphQLResponse response =
        graphQLTestTemplate.postForResource("graphql/translationMap.graphql");

    assertThat(response.isOk()).isTrue();
    assertThat(response.get("$.data.translationMap.locale")).isEqualTo("en");
    List<TranslationMapEntry> entries =
        response.getList("$.data.translationMap.entries", TranslationMapEntry.class);
    assertThat(entries)
