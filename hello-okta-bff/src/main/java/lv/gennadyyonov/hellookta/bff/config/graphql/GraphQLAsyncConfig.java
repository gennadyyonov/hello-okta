...
public class GraphQLAsyncConfig {
  ...
  /**
   * Creates queue for GraphQL async Thread Pool. Extends {@link LinkedBlockingQueue#offer(Object)}
   * to return false always (no queue == no deadlocks).
   */
  private BlockingQueue<Runnable> queue() {
    return new LinkedBlockingQueue<>() {

      /**
       * Do not use the queue. If all the threads are working, then the caller thread should execute
       * the code in its own thread (serially).
       */
      @SuppressWarnings("NullableProblems")
      @Override
  ...
  /**
   * <a
   * href="https://github.com/micrometer-metrics/tracing/wiki/Spring-Cloud-Sleuth-3.1-Migration-Guide#async-instrumentation">Async
   * Instrumentation</a>
   */
  @Bean
  public Instrumentation instrumentation(
      BeanFactory beanFactory, @Qualifier(TASK_EXECUTOR_NAME) ThreadPoolTaskExecutor taskExecutor) {
    return new Instrumentation() {
      ...
      public DataFetcher<?> instrumentDataFetcher(
        ...
        ExecutorService delegate =
            new ContextCopyingDelegateExecutorService(asyncContext, threadPoolExecutor);
