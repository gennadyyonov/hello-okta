...
public class SecurityRoleAspect {
  ...
  @Pointcut(
      "(@annotation(jakarta.annotation.security.PermitAll) || @within(jakarta.annotation.security.PermitAll))")
  ...
  public Object restrictByAllowedUserRoles(ProceedingJoinPoint joinPoint) throws Throwable {
    if (isSecured(joinPoint)
        && !securityService.hasAnyRoles(SecurityConstants.ALLOWED_USERS, new String[0])) {
      String userId = authenticationService.getUserId();
      MethodSignature methodSignature = (MethodSignature) joinPoint.getSignature();
      String message =
          format(USER_HAS_NOT_ACCESS_MESSAGE_FORMAT, userId, getMethodFullName(methodSignature));
      log.error(message);
  ...
  public Object restrictByRole(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    if (annotation != null
        && !securityService.hasAnyRoles(annotation.alias(), annotation.roles())) {
      String userId = authenticationService.getUserId();
      String message =
          format(USER_HAS_NOT_ACCESS_MESSAGE_FORMAT, userId, getMethodFullName(methodSignature));
      log.error(message);
  ...
  private HasRole getHasRoleAnnotation(MethodSignature methodSignature) {
    ...
    return ofNullable(annotation)
        .orElse(
            getMergedAnnotation(methodSignature.getMethod().getDeclaringClass(), HasRole.class));
  }
